...
std::deque<uart_payload> payloads00;
std::mutex mutex00;
...
error_code sys_uart_receive(ppu_thread& ppu, vm::ptr<void> buffer, u64 size, u32 unk)
{
	...
	while (!Emu.IsStopped())
	{
		...
		while (payloads00.size())
		{
			const auto payload = payloads00.front();
