...
{
	sys_hid.trace("sys_hid_manager_ioctl(hid_handle=0x%x, pkg_id=0x%llx, buf=*0x%x, buf_size=0x%llx)", hid_handle, pkg_id, buf, buf_size);
...
{
	sys_hid.trace("sys_hid_manager_check_focus()");
