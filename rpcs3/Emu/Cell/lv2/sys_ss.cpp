...
{
	u8 target_ID = 0xA0;//Gecr
	switch (code)
	{
	case 0x19002://Get Device Type
	{
		sys_ss.warning("sys_ss_appliance_info_manager(code=0x%x (DeviceType), buffer=*0x%x)", code, buffer);
		u8 targetID[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, target_ID};
		memcpy(buffer.get_ptr(), targetID, 16);
		break;
	}
	case 0x19003://Get Device ID
	{
		sys_ss.warning("sys_ss_appliance_info_manager(code=0x%x (IDPS), buffer=*0x%x)", code, buffer);
		u8 idps[] = {0x00, 0x00, 0x00, 0x01, 0x00, target_ID, 0x00, 0x0B, 0x14, 0x00, 0xEF, 0xDD, 0xCA, 0x25, 0x52, 0x66};
	...
	default: sys_ss.error("sys_ss_appliance_info_manager(code=0x%x, buffer=*0x%x) unhandled", code, buffer);
	}
