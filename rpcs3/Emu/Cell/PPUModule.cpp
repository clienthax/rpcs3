...
std::shared_ptr<lv2_prx> ppu_load_prx(const ppu_prx_object& elf, const std::string& path)
{
	...
	for (const auto& prog : elf.progs)
	{
		...
		case 0x700000a8:
			ppu_loader.notice("Old style segment section :o");
			hasSegSym = true;
			break; // 0.85 segment
...
void ppu_load_exec(const ppu_exec_object& elf)
{
	...
	_main->secs.erase(std::remove_if(_main->secs.begin(), _main->secs.end(), [](auto s) { return !(s.type == 1 && s.addr && s.size); }), _main->secs.end());
