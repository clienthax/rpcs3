...
void sysutil_service_manager::on_task() {
	...
	if (std::memcmp(internalCxmlDoc->header.magic, "NPTR", 4) == 0) {//trothy
		...
		cellSysutil.todo("oh no a dodgy packet D:, inb4 explosion");
		cellSysutil.todo("%s", internalCxmlDoc->header.magic);
		cellSysutil.todo("string table cap %d", internalCxmlDoc->stringtable_capacity);
