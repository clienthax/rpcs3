...
void sceVshSysutilService(ppu_thread& ppu) {
	...
	while (!Emu.IsStopped()) {
		...
		if (std::memcmp(m->internalCxmlDoc->header.magic, "NPTR", 4) == 0) {//NP Trophy
			...
			cellSysutil.error("sysutilService magic=%s, str=%s", m->internalCxmlDoc->header.magic, str);


		}
		else if (std::memcmp(m->internalCxmlDoc->header.magic, "NPMG", 4) == 0) {//NP Manager Utility
			const auto& element = vm::_ref<cxml_childelement_bin>(m->internalCxmlDoc->tree.addr() + m->internalCxmlElement->offset + sizeof(cxml_childelement_bin));
			auto str = GetStringAddrFromNameOffset(m->internalCxmlDoc, element.name);
			cellSysutil.error("sysutilService unsupported NPMG packet magic=%s, str=%s", m->internalCxmlDoc->header.magic, str);
			//“managerGetStatus”
		...
		else
		{
			const auto& element = vm::_ref<cxml_childelement_bin>(m->internalCxmlDoc->tree.addr() + m->internalCxmlElement->offset + sizeof(cxml_childelement_bin));
			auto str = GetStringAddrFromNameOffset(m->internalCxmlDoc, element.name);
			cellSysutil.error("sysutilService unsupported packet magic=%s, str=%s", m->internalCxmlDoc->header.magic, str);
		}
	}
}
...
{
	cellSysutil.error("cellSysutilUnregisterCallbackDispatcher");
	return CELL_OK;
}
...
s32 cellSysutilPacketBegin(ppu_thread& ppu, u32 slot, u32 evnt)
{
	...
	if (slot == 0)
	{
		...
		cellSysutil.error("cellSysutilPacketBegin(slot=0x%x, evnt=0x%x) Unhandled slot 0", slot, evnt);
...
s32 cellSysutilPacketEnd(ppu_thread& ppu, u32 slot, u32 evnt)
{
	...
	{
		//fmt::throw_exception("unimplemented slot... deal with slot 0");
		cellSysutil.error("cellSysutilPacketEnd(slot=0x%x, evnt=0x%x) Unhandled slot 0", slot, evnt);
	}
